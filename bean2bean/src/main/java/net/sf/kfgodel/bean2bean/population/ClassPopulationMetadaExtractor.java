 ...
 * @author D. Garcia
 */
public class ClassPopulationMetadaExtractor {

	/**
	 * Mapa de metada de populacion por tipo de populacion y por clase
	 */
	// TODO: Usar softreference si hay problemas de memoria
	private static final Map<PopulationType, Map<Class<?>, ClassPopulationMetadata>> metadatasPerPopulationType = new EnumMap<PopulationType, Map<Class<?>, ClassPopulationMetadata>>(
	 ...
	 *         el atributo
	 */
	public static Annotation[] extractContextAnnotationsFrom(Field field,
			Class<? extends Annotation>[] contextAnnotationsTypes) {
	 ...
	 */
	private static ClassPopulationMetadata createMetadataFor(Class<?> metadataClass, PopulationType populationType) {
		Iterator<Field> allFields = ReflectionUtils.getAllFieldsOf(metadataClass);
		AnnotatedCondition anotadosConInstrucciones = AnnotatedCondition.create(populationType
				.getMetadataAnnotationTypes());
		ConditionalIterator<Field> annotatedFields = ConditionalIterator
				.createFrom(anotadosConInstrucciones, allFields);

		List<PopulationInstruction> instructions = new ArrayList<PopulationInstruction>();
		while (annotatedFields.hasNext()) {
			Field field = annotatedFields.next();
			PopulationInstruction instruction = populationType.createInstructionFor(field);
			instructions.add(instruction);
		}
		ClassPopulationMetadata metadata = ClassPopulationMetadata.create(metadataClass, instructions);
		return metadata;
	}
	 ...
	 */
	private static Annotation[] getAnnotationContextFor(Class<? extends Annotation>[] contextAnnotationsTypes,
			Field field) {
		Annotation[] contextAnnotations = new Annotation[contextAnnotationsTypes.length];
		for (int i = 0; i < contextAnnotationsTypes.length; i++) {
			Class<? extends Annotation> annotationType = contextAnnotationsTypes[i];
	 ...
	 */
	public static ClassPopulationMetadata getMetadataFor(Class<?> metadataClass, PopulationType populationType) {
	 ...
	 */
	private static Map<Class<?>, ClassPopulationMetadata> getMetadataPerClass(PopulationType populationType) {
		Map<Class<?>, ClassPopulationMetadata> metadataPerClass = metadatasPerPopulationType.get(populationType);
		if (metadataPerClass == null) {
			metadataPerClass = new HashMap<Class<?>, ClassPopulationMetadata>();
			metadatasPerPopulationType.put(populationType, metadataPerClass);
		}
	 ...
	 * @param objectFactory
	 */
	private static ConversionInstruction createConversionInstructionFor(PropertyMappingDeclaration mapping,
			ObjectFactory objectFactory) throws CannotCreateConversionInstructionException {

		String alternativeConversor = mapping.getDesignatedConverter();

		ExpressionDeclaration typeDeclaration = mapping.getExpectedTypeDeclaration();
		...
		if (typeDeclaration.isEmpty()) {
			...
			ExpressionDeclaration setterDeclaration = mapping.getSetterExpression();
			String setterExpression = setterDeclaration.getExpressionValue();
		...
		Annotation[] contextAnnotations = mapping.getRelatedAnnotations();
		PopulationType populationType = mapping.getPopulationType();
		GeneralConversionInstruction conversionInstruction = GeneralConversionInstruction.create(typeExtractor,
	 ...
	 */
	public static PopulationInstruction createPopulationInstructionFor(PropertyMappingDeclaration mapping)
			throws CannotCreateConversionInstructionException {
		// This object factory may be null. It's used to create missing beans on a property chain
		ObjectFactory objectFactory = mapping.getActionWhenMissing().getFactoryForMissingObjects();
		GetterInstruction getterInstruction = ScriptedInstruction.createFor(mapping.getGetterExpression(),
				objectFactory);
		ConversionInstruction conversionInstruction = createConversionInstructionFor(mapping, objectFactory);
		SetterInstruction setterInstruction = ScriptedInstruction.createFor(mapping.getSetterExpression(),
				objectFactory);
		MissingPropertyAction whenMissingAction = mapping.getActionWhenMissing();

		GeneralPopulationInstruction populationInstruction = GeneralPopulationInstruction.create(getterInstruction,
				conversionInstruction, setterInstruction, whenMissingAction);
