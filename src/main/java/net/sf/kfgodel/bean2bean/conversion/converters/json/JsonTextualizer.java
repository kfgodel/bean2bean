...
public class JsonTextualizer {
	 ...
	 *             Si se produce un error al deserializar el objeto al tipo indicado
	 */
	public <T> T convertFromStringAsType(final Type expectedType, final String value)
			throws CannotTextUnserializeException {
		try {
			final JavaType expectedJavaType = mapper.constructType(expectedType);
			final T readValue = mapper.readValue(value, expectedJavaType);
			return readValue;
		} catch (final JsonParseException e) {
			throw new CannotTextUnserializeException("Se produjo un error al interpretar el texto json[" + value
					+ "] como el tipo[" + expectedType + "]", e);
